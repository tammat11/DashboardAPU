...
- 8-10: критические финобязательства правильным человеком (платёж/погашение долга/сверка/налоги/зарплата); стратегические решения; работа, спасающая или развивающая компанию.
- 6-7: рутина в рамках роли, но СЛОЖНАЯ (чек-листы, много этапов, несколько участников, ответственность) ИЛИ важная операционная поддержка.
- 5-6: обычная рутина в рамках роли (типовой отчёт, ввод данных, типовая настройка).
- 3-4: простая/мелкая задача с малой ценностью.
- 1-3: нет ценности, можно исключить.

ВАЖНЫЕ МОДИФИКАТОРЫ:
1. ПРОСРОЧКА (статус "Закрыта с просрочкой" или "Открыта просрочена"): +2 к value (макс 10). Просрочка повышает критичность.
2. НЕПОНЯТНАЯ ЗАДАЧА: если описание пустое или название неясное/размытое — ЗАНИЖАЙ value на 1-2 балла. Нельзя высоко оценить то, что непонятно. Это мотивирует людей раскрывать задачи подробно. Обязательно отметь это в reasoning.
3. СЛОЖНОСТЬ: чек-листы, нумерованные пункты, несколько этапов в описании → ближе к верху диапазона роли.

ФИНАНСЫ — не всегда 9-10! Платёж/погашение/сверка/проверка режима = 8-10. Но просто построить отчёт или ввести данные = 5-6 (это лёгкая рутина).
...
{"value": 6, "isRoutine": true, "recommendation": "делать", "reasoning": "краткое обоснование с учётом роли, сложности и понятности задачи", "protocol": "развёрнутый разбор: роль исполнителя → рутина или нет → модификаторы (просрочка/непонятность/сложность) → итоговый балл и рекомендация"}`;

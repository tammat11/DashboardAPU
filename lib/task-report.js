  ...
  try {
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      const payload = await bitrixRequest(baseUrl, 'user.get', {
        ID: ids.slice(i, i + CHUNK),
        ADMIN_MODE: true
      });
      (payload.result || []).forEach((user) => map.set(String(user.ID), user));
    }
...
async function runBitrixTaskReport(baseUrl) {
  ...
  const projectChecklistTaskIds = tasks
    ...
    .filter(Boolean);
  const checklistByTaskId = await fetchTaskChecklists(baseUrl, projectChecklistTaskIds);

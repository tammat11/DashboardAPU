...
async function runBitrixTaskReport(baseUrl) {
  ...
  await Promise.all(Array.from(aupDeptIds).map(async deptId => {
    try {
      const r = await bitrixRequest(baseUrl, 'user.get', { FILTER: { UF_DEPARTMENT: deptId, ACTIVE: true } });
      (r.result || []).forEach(u => allEmployeesMap.set(String(u.ID), u));
    } catch {}
  }));
  const allEmployees = Array.from(allEmployeesMap.values()).map(u => ({
    id: String(u.ID),
    name: getUserDisplayName(u),
    avatar: getUserAvatar(u),
    dept: getUserDepartmentName(u, departmentsMap)
  }));

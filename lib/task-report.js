...
async function fetchAllTasks(baseUrl) {
  ...
  if (firstTasks.length >= TASKS_PAGE_SIZE) {
    const starts = [];
    if (total > TASKS_PAGE_SIZE) {
      for (let s = TASKS_PAGE_SIZE; s < total; s += TASKS_PAGE_SIZE) starts.push(s);
    } else {
      // Unknown total — assume a generous upper bound; empty pages are harmless.
      for (let s = TASKS_PAGE_SIZE; s < TASKS_PAGE_SIZE * 60; s += TASKS_PAGE_SIZE) starts.push(s);
    }
    const baseQuery = encodeBitrixQuery(baseParams);
    // batch allows up to 50 commands each.
    for (let i = 0; i < starts.length; i += 50) {
      const chunk = starts.slice(i, i + 50);
      const cmd = {};
      chunk.forEach(s => { cmd[`p${s}`] = `tasks.task.list?${baseQuery}&start=${s}`; });
      const payload = await bitrixRequest(baseUrl, 'batch', { halt: 0, cmd });
      const results = payload.result?.result || {};
      chunk.forEach(s => {
        const r = results[`p${s}`];
        const pageTasks = r?.tasks || (Array.isArray(r) ? r : []);

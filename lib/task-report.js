...
async function fetchAllTasks(baseUrl) {
  ...
  // First page tells us the total; the rest are fetched in parallel (limited
  // concurrency) instead of one-by-one — sequential paging was the main cause
  // of the /api/task-report 504 timeout.
    ...
    if (total > TASKS_PAGE_SIZE) {
      ...
      for (let i = 0; i < starts.length; i += CONCURRENCY) {
        ...
        const pages = await Promise.all(
          wave.map(start => bitrixRequest(baseUrl, 'tasks.task.list', { ...baseParams, start }))
        );
        pages.forEach(p => tasks.push(...pageTasksOf(p)));
      }
    } else {
      // No usable total — fall back to sequential paging.
      for (let start = TASKS_PAGE_SIZE; ; start += TASKS_PAGE_SIZE) {
        const payload = await bitrixRequest(baseUrl, 'tasks.task.list', { ...baseParams, start });
        const pageTasks = pageTasksOf(payload);
        tasks.push(...pageTasks);
        if (!Array.isArray(pageTasks) || pageTasks.length < TASKS_PAGE_SIZE) break;
      }
    }
  }
...
async function runBitrixTaskReport(baseUrl) {
  ...
  try {
    ...
    // Fetch departments' employees in parallel waves (bounded concurrency to
    // stay friendly to Bitrix) instead of one department at a time.
    const deptIdList = Array.from(aupDeptIds);
    const CONCURRENCY = 5;
    for (let i = 0; i < deptIdList.length; i += CONCURRENCY) {
      const wave = deptIdList.slice(i, i + CONCURRENCY);
      const results = await Promise.all(wave.map(deptId =>
        bitrixRequest(baseUrl, 'user.get', { UF_DEPARTMENT: deptId, ACTIVE: true })
          .then(r => ({ deptId, users: r.result || [] }))
          .catch(() => ({ deptId, users: [] }))
      ));
      results.forEach(({ deptId, users }) => {

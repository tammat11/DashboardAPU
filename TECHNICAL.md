...
Руководитель из актуального поля `UF_HEAD` показывается первым среди сотрудников
своего отдела и выделяется оформлением строки; отдельного дублирующего ФИО в
шапке отдела нет.

Карточка сотрудника показывает полное ФИО, выполненные и общие задачи, задачи
по плану, вне плана и просрочку. Детализация отдельных задач отключена: это
телевизионное представление без интерактивного раскрытия сотрудников.
...
Пользователи с отмеченным boolean-полем Bitrix «Не показывать на телевизоре»
(`UF_USR_1785305377085`) исключаются из списка сотрудников и из задач
телевизионного отчёта автоматически.
